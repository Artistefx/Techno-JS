...
async function choosePlayerPokemon() {
    ...
    return new Promise((resolve) => {
        rl.question('Enter the name of your Pokémon (e.g., pikachu, bulbasaur): ', (answer) => {
            rl.close();
...
function getRandomBotPokemon() {
    const availablePokemons = ['pikachu', 'charmander', 'bulbasaur', 'squirtle', 'jigglypuff', 'eevee', 'gengar', 'snorlax'];
    const randomIndex = Math.floor(Math.random() * availablePokemons.length);
    return availablePokemons[randomIndex];
}

async function startGame() {
    const playerPokemonName = await choosePlayerPokemon(); 
    const botPokemonName = getRandomBotPokemon();
